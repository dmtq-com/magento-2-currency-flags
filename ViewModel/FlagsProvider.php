 ...
 * Provides flags.
 */
class FlagsProvider implements \Magento\Framework\View\Element\Block\ArgumentInterface
{
    /**
     * @var Repository
     */
     ...
     */
    protected function countryCodeToCurrencyCodeMapping()
    {
        return '
...
US Armed Forces,USAF,US Dollar,USD
    ...
    {
        $data = $this->countryCodeToCurrencyCodeMapping();
        $lines = explode("\n", trim($data));

        $result = [];

        foreach (array_slice($lines, 1) as $line) {
            list($country, $countryCode, $currency, $code) = explode(',', $line);
    ...
    protected function currencyCountryMapping(): array
    {
        $data = [];
        foreach ($this->parseData() as $item) {
            $currencyCode = strtoupper($item['code']);
            if (!isset($data[$currencyCode])) {
                $data[$currencyCode] = strtoupper($item['countryCode']);
            }
        }
        return $data;
    }

    public function currencyFlag($currencyCode): string
    {
        $currencyCode = strtoupper($currencyCode);
        $data = $this->currencyCountryMapping();
        $flag = '';
        if (isset($data[$currencyCode])) {
